...
class ScoringAgent:
    ...
    INTENT_POINTS = {
        "Book installation": 20,
        "Request service": 18,
        "Request quote": 15,
        "Request maintenance": 10,
        "Existing customer follow-up": 12,
        "Unclear": 3,
        "Spam or opt-out": 0,
    }
    TIMING_POINTS = {
    ...
    def process(self, intake: IntakeResult, qualification: QualificationResult) -> tuple[int, str]:
        ...
        score = (
            self.URGENCY_POINTS.get(intake.urgency, 0)
            + self.SERVICE_VALUE_POINTS.get(intake.service_type, 0)
            + self.INTENT_POINTS.get(intake.customer_intent, 0)
            + self.TIMING_POINTS.get(intake.timing_need, 0)
            + self._completeness_points(intake)
        )
    ...
    def _completeness_points(self, intake: IntakeResult) -> int:
        if intake.missing_info == "None":
            return 15
        missing_count = len(intake.missing_info.split(";"))
        return max(0, 15 - (missing_count * 6))
